...
trait TableGeneratorTrait {
        ...
        foreach ($reflection->getProperties() as $property) {
            ...
            if ($property->getDeclaringClass()->getName() === static::class) {
                ...
                if (!empty($columnDef)) {
                    ...
                    if (isset($columnDef['isPrimary']) && $columnDef['isPrimary']) {
     ...
     */
    private static function parsePropertyAttributes(ReflectionProperty $property): ?array {
        $result = [
        ...
        // For older PHP versions use PHPDoc comments
            ...
            // Parse column type
            if (preg_match('/@Column\s*\(\s*type\s*=\s*"([^"]+)"/', $docComment, $matches)) {
                $result['type'] = $matches[1];
            }
            
            // Parse length
            if (preg_match('/@Column\s*\([^)]*length\s*=\s*(\d+)/', $docComment, $matches)) {
                $result['length'] = (int)$matches[1];
            }
            
            // Parse NULL/NOT NULL
            if (preg_match('/@Column\s*\([^)]*nullable\s*=\s*(true|false)/', $docComment, $matches)) {
                $result['nullable'] = $matches[1] === 'true';
            }
            
            // Parse default value
            if (preg_match('/@Default\s*\(\s*value\s*=\s*"([^"]*)"/', $docComment, $matches)) {
                $result['default'] = $matches[1];
            } else if (preg_match('/@Default\s*\(\s*value\s*=\s*([^\s\)]+)/', $docComment, $matches)) {
            ...
            // Parse foreign key
            if (preg_match('/@ForeignKey\s*\(\s*reference\s*=\s*"([^"]+)"/', $docComment, $matches)) {
    ...
    private static function buildColumnDefinition(array $columnDef): string {
        ...
        if (isset($columnDef['default'])) {
            ...
            } else if (is_string($default) && !in_array(strtoupper($default), ['NULL', 'TRUE', 'FALSE']) && !is_numeric($default)) {
                $sql .= " DEFAULT '$default'";
            } else if (strtoupper($default) === 'NULL' && (isset($columnDef['nullable']) && $columnDef['nullable'])) {
                $sql .= " DEFAULT NULL";
            } else if (strtoupper($default) === 'TRUE') {
                $sql .= " DEFAULT 1";
            } else if (strtoupper($default) === 'FALSE') {
                $sql .= " DEFAULT 0";
            } else {
                $sql .= " DEFAULT $default";
            }
    ...
    private static function getDefaultCommonFields(): array {
        return [
            ...
            'name' => "  `name` VARCHAR(255) COLLATE utf8_unicode_ci DEFAULT NULL",
    ...
    public static function getTable($classname = null): string {
        ...
        if (PHP_VERSION_ID >= 80000) {
            $tableAttributes = $reflection->getAttributes(\GlpiPlugin\Wazuh\Db\Table::class);
        ...
        if ($tableName === null) {
            ...
            if ($docComment !== false && preg_match('/@Table\s*\(\s*name\s*=\s*"([^"]+)"/', $docComment, $matches)) {
                $tableName = $matches[1];
            }
        }
